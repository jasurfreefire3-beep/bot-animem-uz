...
const targetStart = `if (data === 'btn_profile') {`;
const targetEnd = `    await telegramApiCall('answerCallbackQuery', {
      callback_query_id: id,
      text: \`🔒 \${feature} faqat Animem Pass egalari uchun ochiq!\`,
      show_alert: true,
    });`;

const replacement = `if (data === 'btn_profile') {
...
if (startIndex !== -1 && endIndex !== -1) {
  const fullEndIndex = endIndex + targetEnd.length;
  content = content.substring(0, startIndex) + replacement + content.substring(fullEndIndex);
  fs.writeFileSync('server/bot.ts', content);
  console.log('Successfully replaced handlers precisely in server/bot.ts');
} else {
  console.error('Could not find exact start or end markers in server/bot.ts', { startIndex, endIndex });
}
